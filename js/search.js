...
function displaySuggestions(suggestions, resultsContainer) {
    ...
    suggestions.forEach(suggestion => { // No more index needed
        ...
        div.addEventListener('click', function() {
            searchInput.value = suggestion.suggestionString.split(" (")[0]; // Extract name and secondName
            resultsContainer.style.display = 'none';
            searchInput.focus();
        });
...
searchInput.addEventListener('input', function() {
    ...
    const suggestions = getSuggestions(Database.mainArray, query); // Get the suggestions directly
...
searchInput.addEventListener('keydown', function(event) {
    ...
    if (event.key === 'ArrowDown') {
        ...
        if (selectedSuggestion) {
              ...
              searchInput.value = nameAndSecondName; // Set the input field value
